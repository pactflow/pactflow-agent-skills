...
Shallow-clones pact-foundation/pact-python, runs tree-sitter over the
relevant source files, and writes the DSL reference document consumed by
the pactflow AI skill.

Usage (from repo root):
    uv run --no-project scripts/generate/dsl_python.py
    PACT_PYTHON_REF=v3.3.0 uv run --no-project scripts/generate/dsl_python.py
    uv run --no-project scripts/generate/dsl_python.py --check
"""
...
# ---------------------------------------------------------------------------
# Section builders (all take a repo: Path root of the cloned repo)
# ---------------------------------------------------------------------------


def _parse(repo: Path, rel_path: str) -> tuple[bytes, Node]:
    return _parse_file(repo / "src" / "pact" / rel_path)
...
def _section_pact(repo: Path) -> str:
    source, root = _parse(repo, "pact.py")
...
def _section_match(repo: Path) -> str:
    source, root = _parse(repo, "match/__init__.py")
    public = [
        "int",
        "integer",
        "float",
...
def _section_generate(repo: Path) -> str:
    source, root = _parse(repo, "generate/__init__.py")
    public = [
        "bool",
        "boolean",
        "int",
...
def _section_verifier(repo: Path) -> str:
    source, root = _parse(repo, "verifier.py")
...
# ---------------------------------------------------------------------------
# Examples + matcher table (read from cloned repo)
# ---------------------------------------------------------------------------
...
def _section_matcher_table(repo: Path) -> str:
    source, root = _parse(repo, "match/__init__.py")
    by_name = _module_funcs(source, root)
    primaries = [
        "int",
...
def build_doc(repo: Path) -> str:
    ...
    sections = [
        ...
        "> **Auto-generated** from pact-python source by"
        " `scripts/generate/dsl_python.py` in pact-agent-skills."
        " Do not edit this file directly — run the workflow or the script instead.\n",
        _section_pact(repo),
        _section_interaction(repo),
        _section_match(repo),
        _section_generate(repo),
        _section_verifier(repo),
        _section_examples(repo),
    ]
    return _clean("\n\n".join(s.rstrip() for s in sections) + "\n")


def main() -> int:
    """Clone pact-python, generate dsl.python.md, write or check."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--ref",
        default=os.getenv("PACT_PYTHON_REF", "main"),
        help="Branch/tag to clone (default: $PACT_PYTHON_REF or 'main')",
    )
    parser.add_argument(
        "--output",
        default=str(DEST_PATH),
        help="Output path (default: dsl.python.md in pactflow references)",
    )
    parser.add_argument(
        "--check",
        action="store_true",
