...
def _class_block(source: bytes, root: Node, class_name: str, *, ts: bool = True) -> str:
...
def _section_v3_xml(repo: Path, *, ts: bool = True) -> str:
    ...
    if ts:
        try:
            ...
            extra = f"\n\n{opts_text}"
        except Exception:  # noqa: BLE001 — tree-sitter node traversal can raise many types; skip if absent
            pass
...
def _section_v4(repo: Path, *, ts: bool = True) -> str:
    ...
    if ts:
        ...
        http_ifaces = "\n\n".join(_interface_text(http_src, http_root, n) for n in _HTTP_IFACES)
        msg_ifaces = "\n\n".join(_interface_text(msg_src, msg_root, n) for n in _MSG_IFACES)
        return f"""\
...
def _section_v2_interaction(repo: Path, *, ts: bool = True) -> str:
    ...
    if ts:
        ifaces = "\n\n".join(
            _interface_text(source, root, n) for n in ("RequestOptions", "ResponseOptions", "InteractionObject")
        )
        return f"""\
...
def build_doc(repo: Path, *, ts: bool = True) -> str:
    ...
    sections = [
        ...
        "> **Auto-generated** from pact-js source by"
        " `scripts/generate/dsl_js.py` in pact-agent-skills."
